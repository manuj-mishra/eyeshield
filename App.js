...
export default function App() {
  ...
  const changeValues = (distance) =>  {
    if(distances.length > 20) {
      ...
      setAverages([...averages,(distances.reduce((a,b) => a + b, 0)/distances.length)]);
    } else {
      setDistances([...distances, distance]);
      setAverages([...averages,(distances.reduce((a,b) => a + b, 0)/distances.length)]);
    }  
  ...
  const handleFaces = (facesDict) => {
    if (facesDict.faces.length > 0) {
      changeValues(screenDistance(pythagoras(facesDict.faces[0].leftEyePosition, facesDict.faces[0].rightEyePosition)), distances);
    }
  }

  const pythagoras = (leftEyePosition, rightEyePosition) => {
    return Math.pow(Math.pow(Math.abs(leftEyePosition.x - rightEyePosition.x), 2) + Math.pow(Math.abs(leftEyePosition.y - rightEyePosition.y), 2), 1/2);
  }
          ...
          <ScrollView>
            <View style={[styles.mainContainer, {backgroundColor: colour()}]}>
              ...
              <View style={[styles.displayBox, styles.topBox]}>
                <Text style = {styles.bHeadingText}>
                  Current Distance
                </Text>
              ...
              <View style={styles.displayBox}>
                ...
                <Text style = {styles.bMainText}>
                  {Math.round(distances.reduce((a, b) => a + b, 0)/distances.length)}cm
                </Text>
              </View>
              <View style={styles.displayBox}>
                <Text style = {styles.bHeadingText}>
